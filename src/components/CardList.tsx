...
// create a props called tittle and set tittle to string
const CardList = ( {title}:{title:string}) => {

    // a condtion
    // set list to title then when title is latest transaction the list will be latest transaction else popular content 
    const list = title === "latestTransactions"? latestTransactions : popularContent;
  return (
        ...
        <div className=' flex flex-col gap-2'>
            {list.map(item => (
                <Card key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4">
                  <div className="h-20 w-20 sm:h-16 sm:w-16 rounded-full relative overflow-hidden shrink-0">
                    ...
                    <CardFooter className="ml-auto text-sm font-semibold text-muted-foreground">
